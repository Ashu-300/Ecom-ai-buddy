...
func Connect() {
	...
	if amqpURL == "" {
		...
		if amqpURL == "" {
			log.Fatal("❌ productService AMQP_SERVER_URL not set")
		}
	}

	for {
		var err error
		log.Println("🔁 productService  Connecting to RabbitMQ...")
		conn, err = amqp.Dial(amqpURL)
		if err != nil {
			log.Println("⚠️ productService Failed to connect:", err)
		...
		if err != nil {
			log.Println("⚠️ productService Failed to open channel:", err)
			_ = conn.Close()
			time.Sleep(retryBackoff)
			continue
		}
		...
		log.Println("✅ productService  Connected to RabbitMQ")
		return
	}
}
...
func handleReconnect(nc chan *amqp.Error) {
	err := <-nc
	if err != nil {
		log.Printf("🚨 productService RabbitMQ closed: %v. Reconnecting...", err)
	} else {
		log.Println("ℹ️ productService RabbitMQ NotifyClose returned nil. Reconnecting...")
	}
	...
	for {
		...
		mutex.Unlock()
		if ok {
			log.Println("✅ productService Reconnected to RabbitMQ (background)")
			return
		}
		time.Sleep(retryBackoff)
	}
...
func PublishJSON(queueName string, body []byte) error {
	...
	if err != nil {
		log.Println("❌ productService Publish failed, reconnecting...")
		Connect()
		mutex.Lock()
		ch = channel
		mutex.Unlock()
	...
	log.Printf("📤 productService Sent message to %s", queueName)
	return nil
}
